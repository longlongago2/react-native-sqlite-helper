...
// TODO: 重构：由于jest升级，需要重构模拟 sqliteHelper.js 中的依赖模块
jest.mock('react-native-sqlite-storage', () => {
  // const SQLiteStorage = jest.requireActual('react-native-sqlite-storage');
  // SQLiteStorage.openDatabase.mockResolvedValue('open success'); // 模拟open方法中使用到的依赖方法
  // return SQLiteStorage;
});

// TODO: 升级之前：多层then回调 coverage检测不到代码覆盖率，then只能检测到一层回调，所以多层then回调的统一使用 async/await
...
describe('Test instance method: selectItems', () => {
  test('sqliteHelper selectItems success', async () => {
    ...
    expect(err).toBeUndefined();
    // 某列 有条件
    const { res: res1, err: err1 } = await sqliteHelper.selectItems('people', ['name', 'age', 'sex'], { age: 22 });
    expect(res1).toEqual('SELECT name , age , sex  FROM people WHERE age=22 ;');
    expect(err1).toBeUndefined();
    // 某列 无条件 有页码
    const { res: res2, err: err2 } = await sqliteHelper.selectItems('people', ['name', 'age', 'sex'], null, 1, 5);
    expect(res2).toEqual('SELECT name , age , sex  FROM people limit 5 offset 0;');
    expect(err2).toBeUndefined();
    // 所有列 有条件
    const { res: res3, err: err3 } = await sqliteHelper.selectItems('people', '*', { name: 'mike' });
  ...
  test('sqliteHelper selectItems failed', async () => {
    ...
    expect(err1.message).toEqual('Parameter tableName expects string but object');
    // 参数类型报错2
    const { res: res2, err: err2 } = await sqliteHelper.selectItems('people', {});
    expect(res2).toBeUndefined();
    expect(err2.message).toEqual('Parameter columns expects Array or \'*\' but [object Object]');
    // 执行sql报错
    const { res: res3, err: err3 } = await sqliteHelper.selectItems('people', '*');
    expect(res3).toBeUndefined();
